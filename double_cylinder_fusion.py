...
# generate two cylinders separated at certain distance
# to study the fusion of tubules
...
# from http://data-analytics-tools.blogspot.com/2009/09/reservoir-sampling-algorithm-in-perl.html
def reservoir_sample(list_out,list_in,n_sample):
    list_out_index=[]
    for i,item in enumerate(list_in):
        if i<n_sample:
           list_out.append(item)
           list_out_index.append(i)
        elif i>=n_sample and random.random() < float(n_sample)/float(i+1):
           replace = random.randint(0,len(list_out)-1)
           list_out[replace] = item
           list_out_index[replace] = i 
...
print>>f,"Cylinder radius: %.3f nm" %R
 
L=float(raw_input("Length of cylinder in unit of nm: "))
print>>f, "Cylinder length: %.3f nm" %L

r_dd = float(raw_input("Distance between two cylinder centers, in unit of nm, should be greater than %.3f nm: " %(2.0*R)))
print>>f, "Distance between two cylinder centers: %.3f nm" %r_dd
...
print>>f, "water thickness on one side of the tube: %.3f" %water_thickness

Lz_min = r_dd + 2.0*R 
Lz=float(raw_input("Length of box in z direction, must be greater than %.3f : " %Lz_min))

Ly_min = 2.0*R
Ly=float(raw_input("Length of box in y direction, must be greater than %.3f : " %Ly_min))
...
V_box = Ly*Lz*(L+water_thickness*2.0)*1.0e-27
...
y_min = -Ly/2.0
# set z_min in such a way such that the center of the first tubule is at zero
z_min = -Lz/2.0 + r_dd/2.0
...
# list for storing data of another tubule
another_list=[]
res_num_before_protein=res_num
atm_num_before_protein=atm_num
...
if protein_bool:
              ...
              # zzz += zzz_head 
               ...
               if resnumber!=resnumber_pre:    # for protein the residue number is still determined from the amino acid residue
                  ...
                  if resname.strip() in charges.keys():
                     # the charge is multiplied by two to avoid calculating charge agin for the 2nd tubule
                     chg_tot += 2*charges.get(resname.strip())
               ...
               # store the coordinates for another tubule
               another_list.append([res_num,resname,atm_name,atm_num,xxx,yyy,zzz])
   ...
   # record the res_num and atm_num before generating another tubule
   res_num_before = res_num - res_num_before_protein
   atm_num_before = atm_num - atm_num_before_protein
   # now deal with another tubule
   for item in another_list:
       ...
       # mute water
       mute_water([xxx-x_min,yyy-y_min,zzz-z_min],d_water,water_latt)
       atm_num = atm_num_old + atm_num_before
       res_num = res_num_old + res_num_before
       #gromacs only allow 5 digits for residue number and atom number
       res_num_print=res_num%100000
       atm_num_print=atm_num%100000
       #print it to the gro file 
       print>>g, "%5d%-5s%5s%5d%8.3f%8.3f%8.3f" \
       %(res_num_print,res_name,atm_name,atm_num_print,xxx,yyy,zzz)
       print>>h, "%s %f %f %f" %(atm_name, xxx*10.0,yyy*10.0,zzz*10.0)
...
# clean up the another_list list
another_list=[] 
...
res_num_before_inner = res_num
atm_num_before_inner = atm_num
...
# record the res_num and atm_num before generating another tubule
res_num_before = res_num - res_num_before_inner
atm_num_before = atm_num - atm_num_before_inner
# now deal with another tubule
for item in another_list:
    res_num_old = item[0]
    res_name = item[1]
...
res_num_before_outer = res_num
atm_num_before_outer = atm_num
for i in range(0,n_type_lipid):
...
# record the res_num and atm_num before generating another tubule
res_num_before = res_num - res_num_before_outer
atm_num_before = atm_num - atm_num_before_outer
# now deal with another tubule
for item in another_list:
    res_num_old = item[0]
    res_name = item[1]
    ...
    for ibiny in range(0,wbin_y):
        for ibinz in range(0,wbin_z):
            if water_latt[ibinx][ibiny][ibinz]:
               solvent_site.append([ibinx,ibiny,ibinz])
